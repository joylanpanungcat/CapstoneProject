...
class loginController extends Controller
{
    ...
    public function loginInspector(Request $request){
        $request->validate([
            ...
            $credentials = request(['username']);
            $inspector = inspector::where('username',$credentials)->first();
            ...
            if($inspector['password']== $request->password){
                $tokenResult = $inspector->createToken('Personal Access Token');
                $token = $tokenResult->token;
                if ($request->remember_me)
                    $token->expires_at = Carbon::now()->addWeeks(1);
                $token->save();
    ...
    public function logoutInspector(Request $request)
    {
        $request->user()->token()->revoke();
        return response()->json([
            'message' => 'Successfully logged out'
